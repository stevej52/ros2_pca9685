...
std::shared_ptr<i2cpwmNode> node;
...
static void _set_pwm_frequency(int freq)
{
  ...
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Set PWM frequency to %d Hz ok", freq);

  sleepTime.tv_sec = 0;
  sleepTime.tv_nsec = 5000000L;

  nanosleep(&sleepTime, NULL); // Sleep 5 microseconds.
...
static void _set_active_board(int board)
{
  ...
  if (_active_board != board)
  {
    ...
    if (_pwm_boards[board] < 0)
    {
      ...
      nanosleep(&sleepTime, NULL); // Sleep 5 microseconds, wait for osci.
      ...
      nanosleep(&sleepTime, NULL); // sleep 5microsec, wait for osci
...
static int _load_params(void)
{
  ...
  _config_servo_position(1, 1);

  return 0;
}

/*static int _load_params_orig(void)
{
  ...
  const char * node_namespace = node->get_namespace();

  RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "namespace %s", node_namespace);

  //auto hhh = node->list_parameters();
...
int main(int argc, char **argv)
{
  ...
  //auto node = std::make_shared<i2cpwmNode>("i2cpwm_controller");
  node = std::make_shared<i2cpwmNode>("i2cpwm_controller");

  // globals
  _controller_io_device = 7; // default I2C device on RPi2 and RPi3 = "/dev/i2c-1" Orange Pi Lite = "/dev/i2c-0"
  ...
  _set_pwm_interval(1,0,2048);
  //_set_pwm_interval_all(1,2048);
